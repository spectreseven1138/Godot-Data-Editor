...
func init(_all_data: Dictionary, _plugin: EditorPlugin, _parent_container=null):
	...
	for key in data:
		...
		container.init(key, data[key], plugin, parent_container, plugin.main_panel.data_tree.create_item(parent_container.tree_item) if parent_container else null)
	...
	for i in len(all_data):
		...
		if key == "data":
			...
			for _i in len(all_data[key]):
				ret += "	".repeat(depth + 1)
				ret += '"' + all_data[key].keys()[_i] + '":' + all_values[all_data[key].values()[_i]]["formatted_text"]
